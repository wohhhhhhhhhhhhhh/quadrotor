...
{
    // Ignore transitions involving KEY_ERROR so the first valid frame after startup
    // does not generate false key press/release events.
    if (currentStatus == KeyStatus::KEY_ERROR || lastStatus == KeyStatus::KEY_ERROR) {
...
{
    receiveRxDataFromISR(data, FRAME_SIZE);
}

void VT13RemoteControl::receiveRxDataFromISR(const uint8_t *data, uint16_t length)
{
    if (data == nullptr || length < FRAME_SIZE) return;

    for (uint16_t index = 0; index + FRAME_SIZE <= length; index++) {
        const uint8_t *frame = data + index;
        if (frame[0] != SOF_1 || frame[1] != SOF_2) continue;
        if (!CRCCalculator::verifyCRC16(frame, FRAME_SIZE)) continue;

        memcpy(&m_originalRxDataStorage, frame, FRAME_SIZE);
        m_originalRxDataPointer = &m_originalRxDataStorage;
        m_uartRxTimestamp       = HAL_GetTick();
        m_isConnected           = true;
        m_isDecodeCompleted     = false;
        return;
